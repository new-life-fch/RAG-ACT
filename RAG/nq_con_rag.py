...
def build_nq_con_inputs(
    ...
    for i, ex in enumerate(entries):
        ...
        system_prompt = prompt_dict['qa']['CoN_system']
        user_prompt = prompt_dict['qa']['CoN_user'].format(passages=docs_block, question=question)
...
def run_baseline(model, tokenizer, device, inputs: List[torch.Tensor], gold_answers_list: List[List[str]], max_new_tokens: int) -> Tuple[List[str], dict]:
    preds_baseline: List[str] = []
    with torch.no_grad():
        for input_ids in tqdm(inputs, desc='con_baseline_generate'):
...
def main():
    ...
    inputs, gold_answers_list = build_nq_con_inputs(
    ...
    baseline_sum.update({"alpha": 0.0, "model_name": args.model_name, "intervention": "none", "sample_size": args.sample_size, "prompt": "CoN"})

    baseline_ans_path = os.path.join(ans_dir, 'nq_con_baseline_answers.jsonl')
    baseline_sum_path = os.path.join(sum_dir, 'nq_con_baseline_summary.json')
    ...
    print(
        f"[CoN Baseline] samples={args.sample_size} EM={baseline_sum['EM']:.4f} "
