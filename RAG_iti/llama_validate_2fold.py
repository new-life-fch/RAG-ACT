...
def main(): 
    ...
    for i in range(args.num_fold):
        ...
        if args.use_center_of_mass:
            ...
            com_directions = None
        top_heads, probes, all_head_accs_np = get_top_heads(
            train_set_idxs,
            val_set_idxs,
        ...
        # 探针分数因子：使用验证准确率矩阵 (num_layers, num_heads)
        probe_score_map = all_head_accs_np
        ...
        # Build pyvene intervened model per current fold
        top_heads_by_layer = {}
        ...
        for layer, info_list in top_heads_by_layer.items():
            direction_vec = torch.zeros(num_heads * (model.config.hidden_size // num_heads), dtype=torch.float32)
