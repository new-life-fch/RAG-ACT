...
def tqa_run_answers(frame, engine, tag, preset, model=None, tokenizer=None, verbose=True, device=None, cache_dir=None, interventions={}, intervention_fn=None, instruction_prompt="default", many_shot_prefix=None):
    ...
    with torch.no_grad():
        for idx, input_ids in enumerate(tqdm(tokens, desc="tqa_run_answers")):
            ...
            # Prefer pyvene-wrapped model generation; fall back to HF generate
            gen_out = model.generate(input_ids, top_k=1, max_length=max_len, num_return_sequences=1,)
            if isinstance(gen_out, tuple):
                model_gen_tokens = gen_out[1][:, input_ids.shape[-1]:]
            else:
                model_gen_tokens = gen_out[:, input_ids.shape[-1]:]
            ...
            # (pyvene applies interventions at the model wrapper level)
        ...
        for idx in tqdm(frame.index, desc="tqa_run_probs"):
            if pd.isnull(frame.loc[idx, '{0} lprob max'.format(tag)]):
                ...
                for temp_ans in ref_true:
                    ...
                    # Use pyvene-style forward pass
                    out = model({"input_ids": prompt_ids})
                    outputs = out[1][0].squeeze(0) if isinstance(out, tuple) else out[0].squeeze(0)
...
def run_ce_loss(model_key, model=None, tokenizer=None, device='cuda', interventions={}, intervention_fn=None, num_samples=100): 
    ...
    # pyvene: interventions applied via IntervenableModel; loss via wrapped call
    ...
    with torch.no_grad(): 
        for i in tqdm(rand_idxs, desc="run_ce_loss"):
            ...
            loss = out[0].loss if isinstance(out, tuple) else out.loss
...
def run_kl_wrt_orig(model_key, model=None, tokenizer=None, device='cuda', interventions={}, intervention_fn=None, num_samples=100, separate_kl_device=None): 
    ...
    # pyvene: interventions applied via IntervenableModel; logits via wrapped call
    ...
    with torch.no_grad(): 
        ...
        for i in tqdm(rand_idxs, desc="run_kl_wrt_orig"):
            ...
            if separate_kl_device is not None: 
                ...
                logits_obj = out[1] if isinstance(out, tuple) else out
                orig_logits = logits_obj.logits.cpu().type(torch.float32)
                
            orig_probs = F.softmax(orig_logits, dim=-1)

            out2 = model({"input_ids": input_ids})
            logits2 = (out2[1] if isinstance(out2, tuple) else out2).logits.cpu().type(torch.float32)
